...
class GrammyParser:
    ...
    def clean_grammy_markdown(text: str) -> str:
        ...
        for line in text.splitlines():
            stripped = line.strip()
            if stripped.lower() in EXACT_NOISE_TEXT: continue
            if not stripped:
                if not prev_blank: cleaned_lines.append("")
                prev_blank = True
                continue
    ...
    def normalize_attrs(tag: Tag) -> str:
        ...
        if isinstance(classes, list): values.extend(str(c) for c in classes if c is not None)
        elif classes: values.append(str(classes))
    ...
    def is_noise_text(text: str) -> bool:
        ...
        if not t or t in EXACT_NOISE_TEXT: return True
    ...
    def looks_like_new_article_title(text: str) -> bool:
        ...
        if len(t) < 20 or len(t) > 160 or t.endswith("."): return False
        if "|" in t: return True
    ...
    def is_probable_teaser(tag: Tag, text: str) -> bool:
        ...
        if any(hint in attrs for hint in NOISY_CONTAINER_HINTS): return True
        if len(tag.find_all("a")) >= 3 and len(text) < 180: return True
        if tag.name == "p" and len(text) < 120 and GrammyParser.looks_like_new_article_title(text): return True
        return False
    ...
    def extract_following_blocks_from_title(title_tag: Tag, is_roundup: bool = False) -> list[str]:
        ...
        for elem in title_tag.next_elements:
            if not isinstance(elem, Tag) or elem is title_tag or elem.name not in {"p", "h2", "h3", "h4", "li"}: continue
            text = GrammyParser.normalize_text(elem.get_text(" ", strip=True))
            if not text or text == title_text: continue
            if not is_roundup and started and valid_p_count >= 3 and GrammyParser.looks_like_new_article_title(text): break
            if GrammyParser.is_stop_text(text): break
            if GrammyParser.is_noise_text(text):
                bad_streak += 1
                if started and bad_streak >= 2: break
                continue
            ...
            if elem.name in {"h2", "h3", "h4"}:
                if len(text) >= 8:
                    ...
                    if norm_html not in seen_html:
                        parts.append(str(elem)); seen_html.add(norm_html); started = True; bad_streak = 0
                continue
            ...
            if elem.name == "p":
                ...
                norm_html = re.sub(r"\s+", " ", str(elem).strip())
                if norm_html not in seen_html:
                    parts.append(str(elem)); seen_html.add(norm_html); started = True; bad_streak = 0; valid_p_count += 1
        return parts
    ...
    def extract_grammy_main_content(html: str, url: str) -> str:
        ...
        roundup = any(h in url.lower() or h in title.lower() for h in ["new music friday", "listen to songs", "albums from", "songs & albums from", "best new songs", "best new albums"])
